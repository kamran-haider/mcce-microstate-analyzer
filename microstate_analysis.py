...
class MicrostateAnalysis(object):
    ...
    def generate_byte_indices(self, sample_frequency=100, filename=None):
        ...
        rec_indices = range(start_byte, file_size, sample_frequency * bytes_per_record)
    ...
    def parse_records(self):
        ...
        pbar = ProgressBar(widgets=[Percentage(), Bar(), ETA()], 
                            maxval=self.total_records).start()
        progress_counter = 0
    ...
    def write_hbtxt(self, prefix= ""):
        ...
        with open(hbtxt_file, "w") as t:
            for i in xrange(len(self.residue_list)):
                for j in xrange(len(self.residue_list)):
                    if self.residue_hb_matrix[i, j] / self.total_microstates >= THRESHOLD_TO_WRITE:
                        t.write(
                            "%s\t%s\t%.3f\n" %
                            (self.residue_list[i],
                             self.residue_list[j],
                             self.residue_hb_matrix[i, j] / self.total_microstates))
...
class ConformerHbonds(object):
    ...
    def generate_microstate_hb_network(self, conformer_ids, labels=None):
        """
        Parameters
        ----------
        conformer_ids : TYPE
        ...
        if labels is None:
            nx.relabel_nodes(G, dict(zip(G.nodes(), conformer_ids)), copy=False)
        else:
...
class HydrogenBondedPath(object):
    ...
    def search_for_path(self, hb_network, microstate_index, start, end):
        """
        Parameters
        ----------
        hb_network : TYPE
    ...
    def filter_paths(self):
        ...
        microstate_indices_array = np.asarray(self.microstate_indices)
        shortest_path = None
        filtered_microstates = None
        try:
            min_length_index = np.nanargmin(path_lengths_array)
            shortest_path = path_list_array[min_length_index]
            filtered_microstates = microstate_indices_array[np.where(path_lengths_array == len(shortest_path))]
        except np.ValueError as nopath:
            print nopath
        finally:
            return shortest_path, filtered_microstates
